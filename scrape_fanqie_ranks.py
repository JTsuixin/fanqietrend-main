...
# 番茄榜单 URL 规律: /rank/{channel}_{rankType}_{catId}
# channel: 0=女频, 1=男频; rankType: 1=新书榜, 2=阅读榜
# entry_id 只是进入榜单页的入口分类 ID, 全部分类由页面动态解析
CHANNELS = [
    {"code": 0, "label": "女频", "entry_id": 1139, "prefix": ""},
    {"code": 1, "label": "男频", "entry_id": 1141, "prefix": "男频·"},
]
...
def run_scraper(limit=30, sleep_sec=5):
    ...
    state_file = os.path.join(OUTPUT_DIR, f"task_state_ranks_{date_str}.json")
    ...
    with sync_playwright() as p:
        ...
        for channel in CHANNELS:
            code = channel["code"]
            label = channel["label"]
            name_prefix = channel["prefix"]
            init_url = f"https://fanqienovel.com/rank/{code}_1_{channel['entry_id']}"
            print(f"\n[{datetime.now().strftime('%H:%M:%S')}] ========== 开始抓取{label}新书榜 ==========")
            print(f"[{datetime.now().strftime('%H:%M:%S')}] 正在访问{label}榜单入口页：{init_url}")
            page.goto(init_url, wait_until="load", timeout=15000)
            page.wait_for_selector('a[href^="/page/"]', timeout=5000)

            # 动态解析页面左侧拥有的所有类别目录 (只匹配当前频道的新书榜路由)
            categories_js = """
            (channelCode) => {
                const prefix = `/rank/${channelCode}_1_`;
            ...
            """
            categories = page.evaluate(categories_js, code)
            print(f"✅ 成功自适应提取到 {label} {len(categories)} 个分类标签。开始全量模拟点击抓取下级数据...")

            for cat in categories:
                cat_name = name_prefix + cat["name"]
                cat_href = cat["href"]
                state_key = f"{label}:{cat['name']}"

                if state_key in completed_cats:
                ...
                all_categories.append({
                    "name": cat_name,
                    "channel": "female" if code == 0 else "male",
                ...
                # 更新状态记录
                completed_cats.append(state_key)
